...
export default function Header({ title = 'SSOKTUBE' }: { title?: string }) {
  ...
  const handleNav = (e: React.MouseEvent<HTMLAnchorElement>) => {
    if (typeof window !== 'undefined' && (window as any).__NEXT_CURATOR_UNSAVED__) {
      if (!confirm('Leave without saving? Your analysis will be lost.')) {
        e.preventDefault()
      }
    }
  }
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <nav className="hidden md:flex items-center gap-4 text-sm font-medium">
            ...
            {isStudent ? (
              <>
                <Link href="/mypage" onClick={handleNav} className="text-[var(--text-muted)] hover:text-white transition-colors">Class Materials</Link>
              ...
              <>
                ...
                {isOnSquare
                  ? <Link href="/" onClick={handleNav} className="text-orange-400 hover:text-orange-300 active:opacity-60 active:scale-95 transition-all font-bold select-none">✦ Summarize</Link>
