      ...
      <div className="bg-[var(--bg-base)] border border-orange-500/30 rounded-2xl shadow-2xl px-4 py-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-orange-300">You have review items for today!</p>
              <p className="text-xs text-gray-400 mt-0.5">
                {items.length} questions · {videos.length} videos
              </p>
            </div>
          </div>
        ...
        <div className="mt-3 space-y-1.5">
          {videos.slice(0, 3).map(v => (
            ...
            >
              <span className="text-xs text-white truncate flex-1">{v.videoTitle || 'View Video'}</span>
              <span className="shrink-0 text-[10px] text-orange-400 font-bold ml-2">{v.count} to review →</span>
            </Link>
          ))}
          {videos.length > 3 && (
            <p className="text-[10px] text-gray-500 text-center pt-0.5">+{videos.length - 3} more videos</p>
          )}
        </div>
      </div>
    </div>
